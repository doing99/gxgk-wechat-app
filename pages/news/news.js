...
Page({
  data: {
    ...
    list: [
      ...
      { id: 2, 'type': 'hy', name: '会议通知',storage:[], url: 'news/hy_list.php', enabled: {guest:false, student:false, teacher:true} },
      { id: 3, 'type': 'jz', name: '学术讲座',storage:[], url: 'news/jz_list.php', enabled: {guest:true, student:true, teacher:true} },
      { id: 4, 'type': 'new', name: '综合新闻',storage:[], url: 'news/new_list.php', enabled: {guest:true, student:true, teacher:true} },
    ],
    'active': {
      id: 0,
      'type': 'all',
  ...
  onShow: function(){
    if(app.user.is_bind){
      this.setData({
        user_type: !app.user.is_teacher ? 'student' : 'teacher'
      });
    }else{
      this.setData({
        'active.id': 5,
